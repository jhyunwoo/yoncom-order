...
export const menus = sqliteTable("menus", {
  ...
  menuCategoryId: text("menuCategoryId").notNull().references(() => menuCategories.id),
...
export const tableContexts = sqliteTable("tableContexts", {
  ...
  tableId: text("tableId").notNull().references(() => tables.id),
...
export const tableContextsRelations = relations(tableContexts, ({ one, many }) => ({
  table: one(tables, {
...
export const menuOrders = sqliteTable("menuOrders", {
  id: text("id").primaryKey().notNull().$defaultFn(() => generateId(15)),
  quantity: integer("quantity").notNull(),
  status: text("status").notNull().$type<MenuOrderStatus>().default(menuOrderStatus.PENDING),
