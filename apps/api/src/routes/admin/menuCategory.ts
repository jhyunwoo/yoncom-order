...
// Create Menu
adminMenuCategory.post(
  "/",
  zValidator("json", MenuCategoryRequest.createValidation),
  async (c) => {
...
// Update Menu
adminMenuCategory.put(
  "/",
  zValidator("json", MenuCategoryRequest.updateValidation),
  async (c) => {
    const db = initializeDb(c.env.DB);
...
adminMenuCategory.delete(
  "/",
  zValidator("json", MenuCategoryRequest.removeValidation),
  async (c) => {
    const db = initializeDb(c.env.DB);
