...
type QueryUserResult<T extends QueryUserOption> = (Schema.User & WithTables<T> & WithMenuCategories<T>)[]
...
type QueryMenuCategoryResult<T extends QueryMenuCategoryOption> = (Schema.MenuCategory & WithUser<T> & WithMenus<T>)[]
export const queryMenuCategories = async <T extends QueryMenuCategoryOption> (
  ...
  const menuCategoryIds = menuCategories.map((menuCategory) => typeof menuCategory === "string" ? menuCategory : menuCategory.id);
...
type QueryMenuResult<T extends QueryMenuOption> = (Schema.Menu & WithMenuCategory<T> & WithMenuOrders<T>)[]
export const queryMenus = async <T extends QueryMenuOption> (
  db: DB,
  menus: (Schema.Menu | string)[],
  option?: T
): Promise<QueryMenuResult<T>> => {
  const menuIds = menus.map((menu) => typeof menu === "string" ? menu : menu.id);
...
type QueryTableResult<T extends QueryTableOption> = (Schema.Table & WithUser<T> & WithTableContexts<T>)[];
export const queryTables = async <T extends QueryTableOption> (
  db: DB,
  tables: (Schema.Table | string)[],
  option?: T
): Promise<QueryTableResult<T>> => {
  const tableIds = tables.map((table) => typeof table === "string" ? table : table.id);
...
type QueryTableContextResult<T extends QueryTableContextOption> = (Schema.TableContext & WithTable<T> & WithOrders<T>)[]
export const queryTableContexts = async <T extends QueryTableContextOption> (
  ...
  const tableContextIds = tableContexts.map((tableContext) => typeof tableContext === "string" ? tableContext : tableContext.id);
  ...
  return await db.query.tableContexts.findMany({
    ...
    with: { table, orders },
    orderBy: order ? asc(Schema.tableContexts.createdAt) : desc(Schema.tableContexts.createdAt)
  });
}
...
type QueryOrderResult<T extends QueryOrderOption> = (Schema.Order & WithTableContext<T> & WithMenuOrders<T>)[]
export const queryOrders = async <T extends QueryOrderOption> (
  ...
  const { tableContext, menuOrders, onlyActive, order } = option ?? {};

  if (!onlyActive) 
    return await db.query.orders.findMany({
      where: inArray(Schema.orders.id, orderIds),
      with: { tableContext, menuOrders },
  ...
  else return orders.map((order) => ({ ...result, menuOrders: undefined })) as unknown as QueryOrderResult<T>;
}
...
type QueryMenuOrderResult<T extends QueryMenuOrderOption> = (Schema.MenuOrder & WithOrder<T> & WithMenu<T>)[]
export const queryMenuOrders = async <T extends QueryMenuOrderOption> (
  ...
  const menuOrderIds = menuOrders.map((menuOrder) => typeof menuOrder === "string" ? menuOrder : menuOrder.id);
  ...
  return await db.query.menuOrders.findMany({
    ...
    with: { order, menu },
    orderBy: order ? asc(Schema.menuOrders.createdAt) : desc(Schema.menuOrders.createdAt)
  });
}
...
export const isMenuCategoriesOwnedByUser = (
  ...
  return !menuCategories.some((menuCategory) => menuCategory.userId !== user.id);
}
...
export const isMenusOwnedByUser = (
  ...
  const menuCategoryIds = user.menuCategories.map((menuCategory) => menuCategory.id);
...
export const isTablesOnActivate = (
  ...
  return !tables.some((table) => {
    const recentTableContext = table.tableContexts.sort((a, b) => b.createdAt - a.createdAt).at(0);
...
export const isMenuOrdersOnActivate = (
  menuOrders: Schema.MenuOrder[],
) => {
  return !menuOrders.some((menuOrder) => menuOrder.status !== Schema.menuOrderStatus.PENDING);
}

export const isMenuOrdersOnDeactivate = (
  menuOrders: Schema.MenuOrder[],
) => {
  return !menuOrders.some((menuOrder) => menuOrder.status === Schema.menuOrderStatus.PENDING);
}
...
export const chooseActiveTableContext = (
  ...
  return tableContexts.filter((tableContext) => isTableContextsOnActivate([tableContext])).at(0);
}